$(document).ready(function() {
	...
	$(document).scroll(function() {
		...
		// .condensed
		if ($st < $navHeight) {
			// $pageMain.add($pageFooter).css({"top": 0});
			$nav.removeClass('condensed').removeClass('hidden').css({"top": 0});
			}
		else if ($st > $navHeight) {
			// $pageMain.add($pageFooter).css({"top": jQuery.data($nav, '$navHeightOG')});
			$nav.addClass('condensed').css({"top": -$navHeight});

			// .hidden
			if ($st < $lastScrollTop) {
				$nav.removeClass('hidden').css({"top": 0});
				}
			else {
